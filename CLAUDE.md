...
| `src/runner.js` | Core migration loop: checksum verification, pending filter, Architect Scripting session, calls `up()` per migration, records history |
...
- `runner.test.js` injects a mock Architect Scripting session via the `_archScripting` parameter
